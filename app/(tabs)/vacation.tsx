...
import {
  ...
  StyleSheet, useColorScheme, Modal, TextInput, Alert,
} from 'react-native';
...
export default function VacationScreen() {
  ...
  const [accruals,  setAccruals]  = useState<Record<string,number>>({});
  ...
  const fetchAccruals = async () => {
    const { data } = await supabase.from('profiles').select('id, accrual_rate');
    if (data) {
      const map: Record<string,number> = {};
      data.forEach((p: any) => { map[p.id] = p.accrual_rate; });
      setAccruals(map);
    }
  };
      ...
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        {bal && (
          <View style={styles.balCard}>
            ...
            <Text style={styles.balDays}>
              {bal.remaining}<Text style={styles.balSub}> days left</Text>
            </Text>
        ...
        {myVacs.length === 0 ? (
          <View style={[styles.emptyCard, { backgroundColor: C.card, borderColor: C.border }]}>
            <Text style={{ color: C.muted, textAlign: 'center' }}>No requests yet</Text>
          </View>
        ) : myVacs.map(v => (
          <View key={v.id}
...
const styles = StyleSheet.create({
  ...
  reqBtn:        { backgroundColor: '#E05C2A', borderRadius: 12,
